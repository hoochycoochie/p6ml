...
def grid_search_cv_class_func(df,target_col,feature_cols,param_grid,scoring,model,test_size,random_state,cv,X_train,X_test,y_train,y_test):
    #X_train,X_test,y_train,y_test=train_test_split(df[feature_cols].values,df[target_col].values,test_size=test_size,random_state=random_state)
    ...
    plt.show() 
    #,best_score,rmse,model_name,time_elapsed
    return {'best_params':grid.best_params_,'R2':grid.best_score_,'rmse':'0','model':model.__class__.__name__,'time_elapsed':elapsed}
